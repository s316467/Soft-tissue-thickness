classdef ANOVA
    ...
    methods(Static)
        ...
        function [choosen_landmarks]= analysis_ANOVA (all_data,all_genders, all_ages, landmark_names, title_str)% Carica i tuoi dati
            ...
            title_new = strcat ('ANOVA analysis with age and gender analysis-',title_str);
        ...
        function [choosen_landmarks]= analysis_ANOVA_bmi (all_data,all_genders, all_ages, all_bmi, landmark_names, title_str)% Carica i tuoi dati
                ...
                title_new = strcat ('ANOVA analysis with BMI analysis-',title_str);
                ...
                for i=1:length(landmark_names)
                    % A low p-value indicates that it is unlikely to obtain the observed data if the null hypothesis (i.e., there 
                    % is no difference between the groups) were true.
                    % For example, if there is a significant interaction between Gender and Age, it means that the effect of gender 
                    % on the dependent variable (in this case, Landmark) may differ depending on age.
                ...
                if(isempty(significant_gender_indices) & isempty(significant_age_indices) & isempty(significant_bmi_indices) & isempty(significant_interactionGA_indices) & ...
                        isempty(significant_interactionGB_indices) & isempty(significant_interactionAB_indices) & isempty(significant_interactionGAB_indices))
                    
                    title_new = strcat ('ANCOVA analysis with BMI analysis-',title_str);

                    fprintf("\nANOVA is not sufficient to find some iterations. Trying ANCOVA...\n");
                    for i = 1:size(all_data, 1)
