...
classdef my_plot
    ...
    methods(Static)
        ...
        function plot_distances(all_data_clean, all_genders, all_ages, landmark_names, title_str)
            ...
            f1=figure;
            new_title=strcat('Men distances');
            ...
            f2=figure;
            new_title=strcat('Women distances');
        ...
        function plot_distances_bmi(all_data_clean, all_genders, all_ages, all_bmi, landmark_names, title_str)
         ...
         % Creiamo dei vettori vuoti per ogni combinazione di genere e categoria di età
            ...
            men_under = [men_1_1,men_2_1, men_3_1];
            men_normal = [men_1_2,men_2_2, men_3_2];
            men_over = [men_1_3,men_2_3, men_3_3];
            men_obese = [men_1_4,men_2_4,men_3_4];
            women_under = [women_1_1,women_2_1, women_3_1];
            women_normal = [women_1_2,women_2_2, women_3_2];
            women_over = [women_1_3,women_2_3, women_3_3];
            women_obese = [women_1_4,women_2_4,women_3_4];

            men_under = mean(men_under,2,'omitnan');
            men_normal = mean(men_normal,2,'omitnan');
            men_over = mean(men_over,2,'omitnan');
            men_obese = mean(men_obese,2,'omitnan');
            women_under = mean(women_under,2,'omitnan');
            women_normal = mean(women_normal,2,'omitnan');
            women_over = mean(women_over,2,'omitnan');
            women_obese = mean(women_obese,2,'omitnan');

            men = {men_under, men_normal, men_over, men_obese};
            women = {women_under, women_normal, women_over, women_obese};

            save('men.mat', 'men');
            save('women.mat', 'women');

            f1= figure;
            if(~isempty(men_under))
                plot(1:10, men_under,'DisplayName', ' Underweight' );
            end
            hold on;
            if(~isempty(men_normal))
                plot(1:10, men_normal,'DisplayName', ' Normal weight' );
            end
            hold on;
            if(~isempty(men_over))
                plot(1:10, men_over,'DisplayName', ' Overweight' );
            end
            hold on;
            if(~isempty(men_obese))
                plot(1:10, men_obese,'DisplayName', ' Obese' );
            end
            hold on;
            title('Men mean with BMI factors');
            ...
            xticklabels(landmark_names);
            legend('show');
            saveas(f1,'Men_mean_BMI_withoutAge.png');
            close all;

            f2= figure;
            if(~isempty(women_under))
                plot(1:10, women_under,'DisplayName', ' Underweight' );
            end
            hold on;
            if(~isempty(women_normal))
                plot(1:10, women_normal,'DisplayName', ' Normal weight' );
            end
            hold on;
            if(~isempty(women_over))
                plot(1:10, women_over,'DisplayName', ' Overweight' );
            end
            hold on;
            if(~isempty(women_obese))
                plot(1:10, women_obese,'DisplayName', ' Obese' );
            end
            hold on;
            title('Women mean with BMI factors');
            ...
            xticklabels(landmark_names);
            legend('show');
            saveas(f2,'Women_mean_BMI_withoutAge.png');
            close all;
