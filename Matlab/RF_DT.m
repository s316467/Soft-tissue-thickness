classdef RF_DT
    ...
    function [idx_rf_ab,idx_rf_gb, idx_rf_bmi, idx_rf_gab,idx_dt_bmi,idx_dt_gb, idx_dt_ab,idx_dt_gab, model_bmi, model_gb, model_ab, model_gab]=analysis_RF_DT_BMI(landmark_names, all_data, all_genders, all_ages, all_bmi)
            ...
            % Creazione di oggetti cvpartition per ciascun set di etichette
            n = size(all_data, 2);
            cv_bmi = cvpartition(n, 'LeaveOut', 1);
            cv_combined_gb = cvpartition(n, 'LeaveOut', 1);
            cv_combined_ab = cvpartition(n, 'LeaveOut', 1);
            cv_combined_gab = cvpartition(n, 'LeaveOut', 1);
            ...
            for fold = 1:k
                ...
                y_ab_train_fold = combined_labels_ab(trainIdx_combined_ab);
                y_ab_test_fold = combined_labels_ab(testIdx_combined_ab);
                y_gb_train_fold = combined_labels_gb(trainIdx_combined_gb);
                y_gb_test_fold = combined_labels_gb(testIdx_combined_gb);
                y_gab_train_fold = combined_labels_gab(trainIdx_combined_gab);
                y_gab_test_fold = combined_labels_gab(testIdx_combined_gab);
